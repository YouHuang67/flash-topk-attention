...
class FlashPureScoringFunction(torch.autograd.Function):

    @staticmethod
    def forward(_ctx, q, k, num_heads, score_block_size, topk, padding: Tuple[int, int]):
        B, N, C = q.shape
        H = num_heads
        D = C // H
        ...
        grid = lambda meta: (triton.cdiv(N, meta['Q_BS']), H, B)
        flash_pure_scoring_kernel[grid](
            q_4d, k_4d, Pool_Indices, Pool_Scores,
            B=B, N=N, H=H, D=D,
            IS_POW2=IS_POW2, SCORE_BS=SCORE_BS, SCORE_BS_ORIG=SCORE_BS_ORIG,
            SCORE_K=SCORE_K, SCORE_POOL=SCORE_POOL,
            PAD_HEAD=pad_head, PAD_TAIL=pad_tail, N_PADDED=n_padded,
    ...
    @staticmethod
    def backward(ctx, grad_topk_indices, grad_topk_scores):
        raise NotImplementedError("flash_topk_score_only does not support backward")
...
def flash_topk_score(
    ...
    """Flash Attention with TopK block scoring.

    Args:
        q: Query [B, N, C] or [B, H, N, D] with C = num_heads * D.
        k: Key, same layout as q.
        v: Value, same layout as q. Required when ``score_only=False``.
        ...
        padding: Virtual padding ``(pad_head, pad_tail)``.
        score_only: If True, skip attention output (no V needed), return 2-tuple.
        backend: Kernel backend. Currently only ``"triton"`` is supported.

    Returns:
        If ``score_only=False``:
            (o, topk_indices, topk_scores) — 3-tuple.
        If ``score_only=True``:
            (topk_indices, topk_scores) — 2-tuple.
    """
    if backend != "triton":
        raise ValueError(f"unsupported backend: {backend!r}, must be 'triton'")
    if not score_only and v is None:
        raise ValueError("v is required when score_only=False")
    ...
    if score_only:
        topk_indices, topk_scores = FlashPureScoringFunction.apply(
            q, k, num_heads, score_block_size, topk, padding
        )
        return topk_indices, topk_scores
    else:
...
def _flash_topk_score_naive(
    ...
    Args:
        ...
        padding: ``(pad_head, pad_tail)`` for virtual padding.
        score_only: If True, skip attention output (no V needed), return 2-tuple.
