...
within each Q_BS group; all queries in the block attend over the same merged set.
"""
...
@triton.jit
def _qblock_shared_attn_fwd_kernel(
...
def flash_topk_attn_qblock_shared_candidates(
    q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, topk_indices: torch.Tensor,
    num_heads: int, Q_BS: int, KV_BS: int,
    ...
    Args:
        ...
        topk_indices: [B, H_BI, N, KQ] int32; each value is a KV block id (KV_BS tokens).
        num_heads: H.
        Q_BS: queries per shared-candidate group; N % Q_BS == 0.
        KV_BS: tokens per block id; must match scoring / indices; N % KV_BS == 0.
    ...
    _qblock_shared_attn_fwd_kernel[grid](
...
def _flash_topk_attn_qblock_shared_naive(
    q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, topk_indices: torch.Tensor,
    num_heads: int, Q_BS: int, KV_BS: int, scale: Optional[float] = None,
) -> Tuple[torch.Tensor, torch.Tensor]:
