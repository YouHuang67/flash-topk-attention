"""Reverse-index chunked sparse top-k block attention (forward only)."""
...
def flash_topk_attn_reverse_index_chunked(
    ...
    """Sparse attention with chunked reverse-index to bound mid-buffer memory.

    Args:
        q, k, v: [B, N, C], where C = num_heads * D.
        block_indices: [B, H_BI, N, topk] int tensor, H_BI in {1, H}.
        num_heads: Number of query heads.
        block_size: Tokens per KV block. Must divide N.
        scale: Attention scale. Defaults to 1 / sqrt(D).
        num_kv_heads: Reserved. Must equal num_heads when provided.
        q_bs: Stage1 query batch size.
        chunk_size: Slots per chunk. <=0 enables auto selection.
