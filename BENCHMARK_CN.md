...
| 方法 | 描述 |
|------|------|
| **Naive** | 标准 PyTorch 注意力 + `torch.topk`（两次独立 pass） |
| **Flash Attention 2** | 仅前向注意力，**不含 TopK 计分**（性能下界参考） |
| **Flash Scoring** | 本工作，在单次内核 pass 内同时完成完整注意力输出与 top-k block 索引选取 |

---

## 按序列长度规模汇总

| 规模 | 序列长度 | 配置数 | 对比 Naive 平均加速比 | 对比 Flash Attention 2 平均开销 |
|:-----|:-:|:-:|:-:|:-:|
| Small | 2048 – 4096 | 12 | **4.48×** 加速 | 1.98× |
| Medium | 4096 – 8192 | 25 | **5.21×** 加速 | 1.82× |
| Large | 8192 – 16384 | 25 | **4.80×** 加速 | 2.29× |
| Extra Large | 16384 – 32768 | 14 | — | 3.20× |
| Ultra Large | 32768 – 470016 | 33 | — | 2.88× |

> 序列长度超过 16384 时 Naive 基线因显存不足（OOM）无法运行（原始数据中标记为 SKIPPED）。

---

## 额外开销与 Top-K 的关系

| Top-K | 配置数 | 对比 Flash Attention 2 平均开销 |
|:-----:|:-:|:-:|
| 1 | 3 | 1.42× |
| 2 | 3 | 1.30× |
| 4 | 13 | 2.10× |
...
额外开销随 Top-K 增大而增长，原因是 Bitonic Sort 池维护代价上升。在实际常用范围内（Top-K ≤ 32），开销保持在 Flash Attention 2 的 **1.3–2.5×** 以内。

---

## 2 的幂次 vs 非 2 的幂次 Block Size

| Block Size 类型 | 配置数 | 对比 Flash Attention 2 平均开销 |
|:----------------|:-:|:-:|
| 2 的幂次 | 81 | 2.28× |
| 非 2 的幂次 | 32 | 3.24× |

非 2 的幂次 block size 因内核中的 padding 和边界处理而产生额外开销。

---

## 完整数据

> SKIPPED：Naive 基线因显存不足（OOM）无法运行。

```
规模         | Batch | 序列长度 | 头数 | Block Size | Top-K | Naive (ms) |  FA2 (ms) | Scoring (ms) | Scoring / FA2
----------------------------------------------------------------------------------------------------------------------
...
`*` = 非 2 的幂次 block size · 共 113 个配置 · 错误数：0
